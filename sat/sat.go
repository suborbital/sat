...
func New(config *Config, traceProvider trace.TracerProvider) (*Sat, error) {
	...
	sat.vkServer.HandleHTTP(http.MethodGet, "/meta/message", sat.transport.HTTPHandlerFunc())
	sat.vkServer.GET("/meta/metrics", sat.metricsHandler())
	sat.vkServer.POST("/*any", sat.handler(exec))
...
func (s *Sat) Start() error {
	...
	go func() {
		if err := s.vkServer.Start(); err != nil {
			vektorError <- err
		}
	}()
	...
	s.executor.UseGrav(s.grav)

	if err := s.executor.ListenAndRun(s.config.JobType, s.handleFnResult); err != nil {
	...
	info := process.NewInfo(s.config.Port, s.config.JobType)
	if err := info.Write(s.config.ProcUUID); err != nil {
...
func (s *Sat) Shutdown(ctx context.Context, sig os.Signal) error {
	s.logger.Warn("encountered signal, beginning shutdown:", sig.String())

	// stop Grav with a 3s delay between Withdraw and Stop (to allow in-flight requests to drain)
	// s.vkServer.Stop isn't called until all connections are ready to close (after said delay)
	// this is needed to ensure a safe withdraw from the constellation/mesh
	if err := s.grav.Withdraw(); err != nil {
		s.logger.Warn("encountered error during Withdraw, will proceed:", err.Error())
	}

	time.Sleep(time.Second * 3)

	if err := s.grav.Stop(); err != nil {
		s.logger.Warn("encountered error during Stop, will proceed:", err.Error())
	}

	if err := process.Delete(s.config.ProcUUID); err != nil {
		s.logger.Warn("encountered error during process.Delete, will proceed:", err.Error())
	}

	if err := s.vkServer.StopCtx(ctx); err != nil {
		return errors.Wrap(err, "sat.vektor.StopCtx")
	}

	s.logger.Warn("handled signal, continuing shutdown", sig.String())
	return nil
}
